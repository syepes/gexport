  ...
  pub fn get_orgs(&mut self) -> Result<HashMap<String, String>, anyhow::Error> {
    ...
    if let Ok(c) = reqwest::blocking::Client::builder().user_agent(env!("CARGO_PKG_NAME")).danger_accept_invalid_certs(true).timeout(Duration::from_secs(5)).connection_verbose(true).build() {
      if !self.auth_usr.unwrap().is_empty() && !self.auth_pwd.unwrap().is_empty() {
        ...
        trace!("Auth on {:?} with {:?}/{:?}", req_url, self.auth_usr.unwrap().to_string(), self.auth_pwd.unwrap().to_string());
        ...
        match req.send() {
          Ok(r) => {
            ...
            match r.status() {
              ...
              _ => {
                let msg = match r.json::<serde_json::Value>().unwrap() {
        ...
        match req.send() {
          Ok(r) => {
            ...
            match r.status() {
              StatusCode::OK => {
                match r.json::<serde_json::Value>() {
                  Ok(t) => {
                    ...
                    // debug!("F: {:#?} / D: {:#?}", folder_title.unwrap(), dashboard_title.unwrap());
              ...
              StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => Err(anyhow!("Auth failed")),
              StatusCode::NOT_FOUND => Err(anyhow!("Dashboard not found")),
              _ => {
                let msg = match r.json::<serde_json::Value>().unwrap() {
                  Value::Object(m) => m.get("message").unwrap().to_string(),
                  _ => "unknown".to_string(),
        ...
        match req.send() {
          Ok(r) => {
            ...
            match r.status() {
              StatusCode::OK => {
                match r.json::<serde_json::Value>() {
                  Ok(t) => {
                    ...
                    // debug!("F: {:#?} / D: {:#?}", ds_type.unwrap(), ds_name.unwrap());
              ...
              StatusCode::UNAUTHORIZED | StatusCode::FORBIDDEN => Err(anyhow!("Auth failed")),
              StatusCode::NOT_FOUND => Err(anyhow!("Datasource not found")),
              _ => {
                let msg = match r.json::<serde_json::Value>().unwrap() {
                  Value::Object(m) => m.get("message").unwrap().to_string(),
                  _ => "unknown".to_string(),
